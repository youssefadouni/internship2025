...
class TextExtractor:
    ...
    def extract_text_from_pdf(self, file_path):
        ...
        try:
            ...
            logger.error(f"PDF Processing Error\nFile: {file_path}\nExists: {file_exists}\nReadable: {file_readable}\nError Type: {type(e).__name__}\nDetails: {str(e)}", exc_info=True)
        return text.strip()
    ...
    def extract_text_from_xlsx(self, file_path):
        ...
        try:
            ...
            logger.error(f"Spreadsheet Error\nFile: {file_path}\nSize: {file_size} bytes\nError Type: {type(e).__name__}\nDetails: {str(e)}", exc_info=True)
        return text.strip()
...
def process_single_file_and_save(input_file_path, output_dir_path, silent=False):
    # Get the logger instance defined at the module level
    global logger
    ...
    if silent:
        ...
        # Remove stream handlers to suppress console output
        for handler in list(logger.handlers):
            if isinstance(handler, logging.StreamHandler):
                logger.removeHandler(handler)
        # Add NullHandler if all handlers removed
    ...
    try:
        # Create an instance of TextExtractor to use its methods
        extractor = TextExtractor()
        
        if filename.lower().endswith(".pdf"):
            extracted_text = extractor.extract_text_from_pdf(input_file_path)
        elif filename.lower().endswith(".docx"):
            extracted_text = extractor.extract_text_from_docx(input_file_path)
        elif filename.lower().endswith(".xlsx"):
            extracted_text = extractor.extract_text_from_xlsx(input_file_path)
        elif filename.lower().endswith('.txt'):
            extracted_text = extractor.extract_text_from_txt(input_file_path)
        elif filename.lower().endswith('.csv'):
            extracted_text = extractor.extract_text_from_csv(input_file_path)
        elif filename.lower().endswith('.json'):
            extracted_text = extractor.extract_text_from_json(input_file_path)
        else:
            if not silent:
                logger.warning(f"Unsupported file type for direct extraction: {filename}. No text extracted.")
            extracted_text = ""

        output_file_path = os.path.join(output_dir_path, "extracted_content.txt")
        with open(output_file_path, "w", encoding="utf-8") as f:
            f.write(extracted_text if extracted_text else "")
        
        if not silent:
            if extracted_text and success_extraction:
                logger.info(f"Successfully extracted text from {filename} to {output_file_path}")
            elif not success_extraction:
                logger.warning(f"Extraction from {filename} resulted in an error message written to {output_file_path}")
            else:
                logger.warning(f"No text extracted from {filename}. Output file created: {output_file_path}")
        ...
        try:
            with open(output_file_path, "w", encoding="utf-8") as f:
                f.write(f"Error during extraction: {str(e)}")
        except Exception as e_write:
            if not silent:
                logger.error(f"Additionally, failed to write error to output file {output_file_path}: {e_write}")
        success_extraction = False
    finally:
        if silent and original_handlers:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract text from a single file and save to output_dir/extracted_content.txt.")
    parser.add_argument("--input_file", required=True, help="Path to the input file.")
    parser.add_argument("--output_dir", required=True, help="Directory to save the extracted text file.")
    parser.add_argument("--silent", action="store_true", help="Suppress console logging from this script.")
    
    args = parser.parse_args()

    if args.silent:
        # Use the custom logger variable instead of the root logger
        for handler in list(logger.handlers):
            if isinstance(handler, logging.StreamHandler):
                logger.removeHandler(handler)
        if not logger.hasHandlers():
    ...
    if not args.silent:
        try:
            # Assuming pytesseract is imported at the top of the file
            tesseract_version = pytesseract.get_tesseract_version()
            logger.info(f"Tesseract version {tesseract_version} found.")
        except NameError: # Handle if pytesseract wasn't imported
             logger.error("pytesseract library not found or not imported. PDF OCR will not be available.")
        except pytesseract.TesseractNotFoundError:
            logger.error("Tesseract is not installed or not found in your PATH.")
            logger.error("Please install Tesseract for PDF OCR capabilities.")

    if not args.silent:
        logger.info(f"Starting single file extraction for: {args.input_file} -> {os.path.join(args.output_dir, 'extracted_content.txt')}")
    ...
    if not os.path.exists(args.output_dir):
        try:
            ...
            print(f"Error: Could not create output directory {args.output_dir}. Aborting.", file=sys.stderr)
            sys.exit(1)
            
    if not os.path.isdir(args.output_dir):
        if not args.silent:
            logger.error(f"Output directory path {args.output_dir} is not a directory.")
        print(f"Error: Output directory path {args.output_dir} is not a directory. Aborting.", file=sys.stderr)
        sys.exit(1)

    success = process_single_file_and_save(args.input_file, args.output_dir, args.silent)

    if success:
        if not args.silent:
            print(f"Text extraction successful. Output saved to {os.path.join(args.output_dir, 'extracted_content.txt')}")
        sys.exit(0)
    else:
        if not args.silent:
            print(f"Error: Text extraction failed. Check logs or {os.path.join(args.output_dir, 'extracted_content.txt')} for error details.", file=sys.stderr)
        sys.exit(1)